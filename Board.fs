...
type ActiveBoard (sideLength : int) =
    ...
    let Neighbors (x : int, y : int) = 
        ...
        |> List.filter (fun (x, y) -> x >= 0 && x < sideLength && y >= 0 && y < sideLength)
    ...
    let rec GetLiberties ((x : int, y : int)) =
        if GetCellState (x, y) = CellState.Empty then []
            ...
            /// List of cells adjacent to the current one. 
            /// Only cells which are empty or occupied by the same color stone are listed.
            let neighborList = 
                (x, y) 
                |> Neighbors 
                |> List.filter (fun n -> GetCellState n = CellState.Empty || GetCellState n = currentState)

            /// List of liberties (empty cells) available to the current cell's stone.
            let currentLiberties = neighborList |> List.filter (fun n -> GetCellState n = CellState.Empty)

            /// Trimmed version of neighborList that only contains non-empty cells which have not been visited by GetLiberties.
            let currentNeighbors = neighborList |> List.filter (fun n -> GetVisitState n <> VisitState.Visited)
